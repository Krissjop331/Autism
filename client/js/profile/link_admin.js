document.addEventListener("DOMContentLoaded", () => {
    ...
    let adminLinksContainer = document.createElement('div');
    let userData = JSON.parse(localStorage.getItem('user')) || [];
    adminLinksContainer.classList.add('admin-links-container');

    if (userData.role == 'admin') {
        let linkAdmin = document.createElement('div');
        ...
        linkAdmin.style.color = '#fff';
        menuContent.appendChild(linkAdmin);

        linkAdmin.addEventListener('click', () => {
            let adminLinksDiv = document.createElement('div');
            adminLinksDiv.classList.add('admin-links');

            let createNewsLink = document.createElement('a');
            createNewsLink.href = '/admin/create-news';
            createNewsLink.textContent = 'Create News';

            let createCourseLink = document.createElement('a');
            createCourseLink.href = '/admin/create-course';
            createCourseLink.textContent = 'Create Course';

            let allNewsLink = document.createElement('a');
            allNewsLink.href = '/admin/all-news';
            allNewsLink.textContent = 'All News';

            let allCoursesLink = document.createElement('a');
            allCoursesLink.href = '/admin/all-courses';
            allCoursesLink.textContent = 'All Courses';

            let allUsersLink = document.createElement('a');
            allUsersLink.href = '/admin/all-users';
            allUsersLink.textContent = 'All Users';

            adminLinksDiv.appendChild(createNewsLink);
            adminLinksDiv.appendChild(createCourseLink);
            adminLinksDiv.appendChild(allNewsLink);
            adminLinksDiv.appendChild(allCoursesLink);
            adminLinksDiv.appendChild(allUsersLink);

            menuContent.classList.toggle("admin-menu")
            let menu = document.querySelector('.menu');
            if (menuContent.classList.contains("admin-menu") && menu.getAttribute('data-close') === 'false') {
                adminLinksContainer.innerHTML = '';
                adminLinksContainer.appendChild(adminLinksDiv);
                menuContent.appendChild(adminLinksContainer)
            } else {
                if (document.querySelector('.admin-links-container'))
                    menuContent.removeChild(adminLinksContainer)
            }
        });
    }
})